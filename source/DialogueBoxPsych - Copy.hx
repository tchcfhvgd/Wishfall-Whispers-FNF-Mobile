...
class DialogueCharacter extends FlxSprite
{
	...
	public static var DEFAULT_CHARACTER:String = 'lilac';
...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!dialogueEnded) {
			...
			} else if(daText == null || daText.finishedText) {
			...
			if(box != null || box.animation.curAnim.finished) {
				for (i in 0...textBoxTypes.length) {
					var checkArray:Array<String> = ['', 'center-'];
					var animName:String = box.animation.curAnim.name;
					for (j in 0...checkArray.length) {
		...
		} else if (dialogueEnded){ //Dialogue ending
