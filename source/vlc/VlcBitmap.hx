...
class VlcBitmap extends Bitmap
{
	...
	{
		trace(source + "cacapeepee");

		trace("me cago" + source);

		if(source == "wipeIn.mp4") trace("me gusta" + source.toString);
	...
	function videoInitComplete()
	{
		...
		//bitmapData2 = new BitmapData(Std.int(videoWidth), Std.int(videoHeight), true, 0);
				...
				try
				{
					...
					if (bufferMem != null)
					{
						...
						if (libvlc.getPixelData() != null){
							...
							var bitmapDataCache:BitmapData = null;
							if(bitmapDataCache != null)
								...
								//trace(videoName.toString() + " melon");

								if(videoName.contains('wipeIn.mp4') || videoName.contains('wipeOut.mp4'))
									{
										bitmapData.threshold(bitmapData, frameRect, frameRect.topLeft, ">=",  0xFF111111, 0x00FF5454, 0xFFFFFFFF, true);
										//trace(bitmapData.threshold(bitmapData, frameRect, frameRect.topLeft, ">=",  0xFF111111, 0x00FF5454, 0xFFFFFFFF, true));
										//trace("wipes lmao" + videoName);
								...
								if(bitmapData != null)
									...
									if (true==false){
							//var caca:Graphics = new bitmapData.Graphics();
							//videoWidth * videoHeight * 4
							//bitmapData.scroll(-200,-550);
							bitmapData.setPixels(frameRect, Bytes.ofData(bufferMem));
							//  caca = bitmapData;
							// var customGraphics = libvlc.Graphics();
							// drawGraphicsData(graphicsData:Vector<IGraphicsData>):Void
							bitmapData.scroll(-200,-550);
							
							bitmapDataCache = bitmapData;
							//bitmapData.disposeImage();
							//if(videoName == 'wipeIn' || videoName == 'wipeOut')
							//bitmapDataCache.draw(bitmapData, MULTIPLY, false);
							//bitmapData.disposeImage();
							//bitmapDataCache.disposeImage();
							//bitmapData.disposeImage();
							//var pp:Shape = new Shape();
							 //bitmapDataCache = bitmapData;
							// pp.graphics.drawRect(0,0, 100, 500);
							// pp.graphics.lineStyle (2, 0x990000, .75);
							//pp.graphics.beginBitmapFill(bitmapData, false,false);
							
							
							// if(videoName == 'wipeIn' || videoName == 'wipeOut')
							// pp.blend = MULTIPLY;
							//bitmapData.draw(bitmapDataCache, MULTIPLY, false);
							//bitmapDataCache = bitmapData;
							//bitmapData = bitmapData;
							//pp.graphics.clear();
							
							//bitmapData.remove(bitmapData);
						//	bitmapData = null;
							...
							// var caca:Graphics = Graphics.beginBitmapFill();
							// caca.beginBitmapFill(bitmapData, false,false);

							// function acacacacac(blend:String):Graphics {
							// 	return 


						//bitmapData.setPixels(frameRect, Bytes.ofData(bufferMem));
						// if(videoName == 'wipeIn' || videoName == 'wipeOut')
						// bitmapData.image.blendMode(MULTIPLY);
						//bitmapData.draw(bitmapData, MULTIPLY, false);
						
						
					//var newGraphic:FlxGraphic = FlxGraphic.fromBitmapData(bitmapData, false, false);
					
					//FlxG.bitmap.addGraphic(newGraphic);
					//loadGraphic(newGraphic);
						//FlxGraphic.beginBitmapFill(bitmapData.image, false, false);
						// bitmapData.blendMode = 
						// var caca:FlxObject; 
						// caca.blendMode(MULTIPLY);
						// bitmapData.transform = caca.transform;
						//blendModeFromString('MULTIPLY');
						//bitmapData.BlendMode(MULTIPLY);



						// var renderer = new OpenGLRenderer(Lib.current.stage.context3D, bitmapData);
						// if(videoName == 'wipeIn' || videoName == 'wipeOut')
						//  renderer.__overrideBlendMode = "MULTIPLY";
			
						// __drawGL(bitmapData, renderer);





							//bitmapData.draw(bitmapData, null, null, 'MULTIPLY' ,null, false);
