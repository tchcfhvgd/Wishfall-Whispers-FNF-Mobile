...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		loadChar(!daAnim.startsWith(''), false);
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bg:BGSprite = new BGSprite('covemidalt', -600 + OFFSET_X - playerXDifference, -300, 0.9, 0.9);
			bgLayer.add(bg);
	...
	function addSettingsUI() {
		...
		check_player.checked = daAnim.startsWith('');
	...
	function addCharacterUI() {
		...
		imageInputText = new FlxUIInputText(15, 30, 200, 'characters/lilac', 8);
		...
		fpsCheckBox = new FlxUICheckBox(healthIconInputText.x + 200, healthIconInputText.y, null, null,"", 50);
		fpsCheckBox.checked = !fpsCheckBox.checked;
		fpsCheckBox.callback = function() {
			char.originalDynamicFPS = !char.originalDynamicFPS;
			char.dynamicFPS = char.originalDynamicFPS;
		};
		...
		tab_group.add(new FlxText(fpsCheckBox.x + 20, fpsCheckBox.y, 0, 'Dynamic FPS:'));
	...
	function reloadCharacterImage() {
		...
		if(char.animationsArray != null && char.animationsArray.length > 0) {
			...
			char.quickAnimAdd('danceRight', 'danceRight');
		}
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			fpsCheckBox.checked = char.originalDynamicFPS;
			noAntialiasingCheckBox.checked = char.noAntialiasing;
			resetHealthBarColor();
			leHealthIcon.changeIcon(healthIconInputText.text);
	...
	function saveCharacter() {
		var json = {
			...
			"dynamicFPS": char.originalDynamicFPS,
