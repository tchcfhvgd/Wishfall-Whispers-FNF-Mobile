...
class WeekEditorBState extends MusicBeatState
{
	...
	override function create() {
		...
		txtWeekTitle.setFormat("a Anti Corona", 32, FlxColor.WHITE, RIGHT);
	...
	function addEditorBox() {
		...
		var freeplayButton:FlxButton = new FlxButton(0, 650, "Freeplay", function() {
			MusicBeatState.switchState(new WeekEditorBFreeplayState(weekFile));
...
class WeekEditorBFreeplayState extends MusicBeatState
{
	var weekFile:WeekFile = null;
	public function new(weekFile:WeekFile = null)
	{
	...
	function addEditorBox() {
		...
		var storyModeButton:FlxButton = new FlxButton(0, 685, "Story Mode", function() {
			MusicBeatState.switchState(new WeekEditorBState(weekFile));
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 685, "Save Week", function() {
			WeekEditorBState.saveWeek(weekFile);
		});
		saveWeekButton.screenCenter(X);
		saveWeekButton.x += 120;
		add(saveWeekButton);
	...
	override function update(elapsed:Float) {
		if(WeekEditorBState.loadedWeek != null) {
			super.update(elapsed);
			FlxTransitionableState.skipNextTransIn = true;
			FlxTransitionableState.skipNextTransOut = true;
			MusicBeatState.switchState(new WeekEditorBFreeplayState(WeekEditorBState.loadedWeek));
			WeekEditorBState.loadedWeek = null;
			return;
		}
